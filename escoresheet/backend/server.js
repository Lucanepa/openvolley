...
`.trim()
  ...
  const WHISTLE_SVG = `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2 16C2 16 3.5 14.5 5 14.5C6.5 14.5 7 15.5 8.5 15.5C10 15.5 10.5 14.5 12 14.5C13.5 14.5 14 15.5 15.5 15.5" stroke="#94a3b8" stroke-width="1.5" stroke-linecap="round"/><circle cx="9" cy="9" r="6" stroke="#e2e8f0" stroke-width="2"/><path d="M13.5 5.5L20 2" stroke="#e2e8f0" stroke-width="2" stroke-linecap="round"/><circle cx="9" cy="9" r="2" fill="#3b82f6"/></svg>`
  if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '')) {
    ;(async () => {
      try {
        ...
        const indoorRoles = [
          { key: 'referee', label: 'Referee', path: '/referee', color: '#3b82f6', icon: 'whistle' },
        ...
        const beachRoles = [
          { key: 'beach_referee', label: 'Referee', path: '/beach-referee', color: '#3b82f6', icon: 'whistle' },
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .card {
      ...
      transition: transform 0.15s; width: 240px; flex-shrink: 0;
    }
    ...
    .card .qr { background: #fff; border-radius: 10px; padding: 10px; display: inline-block; margin-top: 12px; }
    .card .qr svg { display: block; width: 140px; height: 140px; }
...
<body>
  ...
  <div class="section">
    ...
    <div class="grid">
      ${indoorQR.map(r => `
      <div class="card">
        <div class="icon">${r.icon === 'whistle' ? WHISTLE_SVG : r.icon}</div>
