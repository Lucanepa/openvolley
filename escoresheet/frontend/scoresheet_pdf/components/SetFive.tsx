...
const PointBox: React.FC<{ num: number; filledState?: 0 | 1 | 2 }> = ({ num, filledState = 0 }) => {
    return (
        <div 
                    ...
                    return (
                        <div key={colIdx} className="border-r border-black flex flex-col h-full" style={{ width: '10mm' }}>
                            <div className="border-b border-black shrink-0 p-0.5 flex items-center justify-center" style={{ height: '0.5cm' }}>
                                <div className="text-[10px] text-center font-bold">{sub1 ? sub1.playerIn : ''}</div>
                            </div>
...
const PointsColumn30: React.FC<{ isLast?: boolean, currentScore?: number, timeouts?: [string, string], markedPoints?: number[] }> = ({ isLast, currentScore = 0, timeouts = ["", ""], markedPoints = [] }) => {
    return (
        <div className={`flex flex-col shrink-0 ${isLast ? '' : 'border-l-0 border-black'}`} style={{ width: '15mm', height: '3.5cm' }}>
            ...
            <div className="bg-white flex flex-col items-center justify-start py-1 shrink-0 " style={{ height: '1cm' }}>
                <span className="text-[8px] font-bold leading-none  " style={{ height: '0.5cm' }}>"T"</span>
                <div className="flex flex-col w-full px-2 items-center   ">
                    <div className="w-full text-center text-[10px] font-bold bg-white leading-none flex items-center justify-center" style={{ height: '0.5cm' }}>{timeouts[0] || ":"}</div>
                    <div className="w-full text-center text-[10px] font-bold bg-white leading-none flex items-center justify-center" style={{ height: '0.5cm' }}>{timeouts[1] || ":"}</div>
...
export const SetFive: React.FC<SetFiveProps> = ({
 ...
 {/* End Time */}
           ...
           <div className="border-r border-black flex items-center justify-between px-2 bg-white shrink-0" style={{ width: '19.7mm', marginLeft: '3mm' }}>
                <div className="flex items-center gap-1 w-full">
                    <div className="flex items-center gap-1">
                        <div className="w-6 h-6 rounded-full border border-black text-center bg-gray-200 text-black font-bold text-sm shrink-0 flex items-center justify-center"></div>
                    </div>
